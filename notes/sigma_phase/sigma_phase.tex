...
\eqar{
  V_0=&V_z\\
  V_{\pm1}=&\mp\frac{1}{\sqrt{2}}\paren{V_x\pm\ui V_y}
}

This should also be the polarization basis used for the $\pi$
and $\sigma^{\pm}$ polarizations.
\eqar{
  \hat e_\pi=&\hat e_z\\
  \hat e_{\sigma^\pm}=&\mp\frac{1}{\sqrt{2}}\paren{\hat e_x\pm\ui\hat e_y}
}
Or the amplitude relation,
\eqar{
  A_z=&A_\pi\\
...
\eqar{
  A_\pi=&A_z\\
  A_{\sigma^+}=&\frac{1}{\sqrt{2}}\paren{\ui A_y-A_x}\\
  A_{\sigma^-}=&\frac{1}{\sqrt{2}}\paren{\ui A_y+A_x}
}
