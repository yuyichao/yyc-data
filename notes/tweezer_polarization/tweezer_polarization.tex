...
\parindent=0pt
\title{Transverse circular polarization in tightly focused beam}
...
\section{Goal}
Trying to accurately understand the origin of the transverse circular polarization
...
Full quantitative understanding of the transverse circular polarization
...
that the axial polarization does exist.
We can therefore break down the proof of the existance of
the transverse circular polarization in two steps.
\begin{enumerate}
\item Axial polarization also exist on the focal plane.
\item The axial polarization has to be out-of-phase with the transverse polarization.
\end{enumerate}
These two would guarantee a transverse circular polarization.
...
\eqar{
  ...
  \begin{split}
    ...
    =&-\nabla^2\vec E
  \end{split}\\
  \nabla^2\vec E=&\frac{1}{c^2}\pdiff[2]{\vec E}{t}
}

Scalar wave equation for,
\eqar{
  \nabla^2u=&\frac{1}{c^2}\pdiff[2]{u}{t}
}
If the wave is monotonic, we can write it out as
$u(\vec r, t)=u_0(\vec r)\ue^{\ui\omega t}$, in which case the wave equation becomes,
\eqar{
  \nabla^2u_0=&k^2 u_0
}
where $k\equiv\dfrac{\omega}{c}$.
We can define the flux of the wave $\vec j\equiv u_0^*\nabla u_0-u_0\nabla u_0^*$,
we have,
\eqar{
  \begin{split}
    \nabla\cdot\vec j=&\nabla\cdot\paren{u_0^*\nabla u_0-u_0\nabla u_0^*}\\
