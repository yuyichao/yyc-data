...
function add_mode_offset!(sys, mode_idx, δ)
    mode = sys.modes[mode_idx]
    mode = @set mode.ω = mode.ω + δ
    sys.modes[mode_idx] = mode
    return sys
end
add_mode_offset_callback(mode_idx) =
    (sys, δ)->add_mode_offset!(sys, mode_idx, δ)

function add_param_offset!(sys, pulse_idx; offsets...)
    pulse = sys.pulses[pulse_idx]
    :τ in keys(offsets) && (pulse = @set(pulse.τ = pulse.τ + values(offsets).τ))
    :dΩ in keys(offsets) && (pulse = @set(pulse.dΩ = pulse.dΩ + values(offsets).dΩ))
    :Ω′ in keys(offsets) && (pulse = @set(pulse.Ω′ = pulse.Ω′ + values(offsets).Ω′))
    :dφ in keys(offsets) && (pulse = @set(pulse.dφ = pulse.dφ + values(offsets).dφ))
    :ω in keys(offsets) && (pulse = @set(pulse.ω = pulse.ω + values(offsets).ω))
    sys.pulses[pulse_idx] = pulse
    return sys
end
add_param_offset_callback(pulse_idx, name) =
    (sys, δ)->add_param_offset!(sys, pulse_idx; name=>δ)

function max_δ(sys)
    max_pω = maximum(pulse.ω for pulse in sys.pulses)
    min_pω = minimum(pulse.ω for pulse in sys.pulses)
    max_sω = maximum(mode.ω for mode in sys.modes)
    min_sω = minimum(mode.ω for mode in sys.modes)

    return max(abs(max_pω - min_sω), abs(max_sω - min_pω))
end
max_τ(sys) = maximum(abs(pulse.τ) for pulse in sys.pulses)
...
struct SimpleMultiResult
    τ::Float64
    dis::Vector{Complex{Float64}}
    area::Float64
    cumdis::Vector{Complex{Float64}}
    disδ::Vector{Complex{Float64}}
...
@testset "Random multi-mode sequence" begin
    ...
    function eval_grad(sys, sys_cb, nh)
        function eval_wrapper(δ)
            ...
            sys′.modes .= sys.modes
            resize!(sys′.pulses, length(sys.pulses))
            sys′.pulses .= sys.pulses
            sys_cb(sys′, δ)
            SL.compute!(sys′)
            return SimpleMultiResult(sys′.result.τ, copy(sys′.result.dis),
                                     sys′.result.area, copy(sys′.result.cumdis),
                                     copy(sys′.result.disδ), copy(sys′.result.areaδ))
        end
        h = nh / 4
        hs = (-4, -3, -2, -1, 1, 2, 3, 4) .* h
        results = eval_wrapper.(hs)
        return SimpleMultiResult(compute_grad((x->x.τ).(results)..., h),
    ...
    function test_nseg(nseg, nmode)
        resize!(sys.modes, nmode)
        sys.modes .= rand.(Ref(all_modes_array))
        resize!(sys.pulses, nseg)
        sys.pulses .= rand.(Ref(all_params_array))
        SL.compute!(sys)

        nhτ = 0.005 / max(max_δ(sys), 1.0)
        nhΩ = 0.005
        nhΩ′ = 0.005
        nhδ = 0.005 / max(max_τ(sys), 1.0)
        nhφ = 0.005

        grad_mode = [eval_grad(sys, add_mode_offset_callback(mode_idx), nhδ)
                     for mode_idx in 1:nmode]

        grad_pulses = [[eval_grad(sys, add_param_offset_callback(i, :τ), nhτ)
                        for i in 1:nseg],
                       [eval_grad(sys, add_param_offset_callback(i, :dΩ), nhΩ)
                        for i in 1:nseg],
                       [eval_grad(sys, add_param_offset_callback(i, :Ω′), nhΩ′)
                        for i in 1:nseg],
                       [eval_grad(sys, add_param_offset_callback(i, :dφ), nhφ)
                        for i in 1:nseg],
                       [eval_grad(sys, add_param_offset_callback(i, :ω), nhδ)
                        for i in 1:nseg]]

        for mode_idx in 1:nmode
            @test(sys.result.disδ[mode_idx] ≈ -grad_mode[mode_idx].dis[mode_idx],
                  rtol=1e-8, atol=1e-8)
            @test(sys.result.areaδ[mode_idx] ≈ -grad_mode[mode_idx].area,
                  rtol=1e-7, atol=1e-7)
        end

        for pulse_idx in 1:nseg
            for j in 1:5
                @test(sys.result.τ_grad[pulse_idx][j] ≈ grad_pulses[j][pulse_idx].τ,
                      rtol=1e-8, atol=1e-8)
                @test(sys.result.area_grad[pulse_idx][j] ≈
                    grad_pulses[j][pulse_idx].area,
                      rtol=1e-3, atol=1e-3)
                for mode_idx in 1:nmode
                    @test(sys.result.dis_grad[mode_idx][pulse_idx][j] ≈
                        grad_pulses[j][pulse_idx].dis[mode_idx],
                          rtol=1e-7, atol=1e-7)
                    @test(sys.result.cumdis_grad[mode_idx][pulse_idx][j] ≈
                        grad_pulses[j][pulse_idx].cumdis[mode_idx],
                          rtol=1e-5, atol=1e-5)
                    @test(sys.result.disδ_grad[mode_idx][pulse_idx][j] ≈
                        grad_pulses[j][pulse_idx].disδ[mode_idx],
                          rtol=1e-5, atol=1e-5)
                    @test(sys.result.areaδ_grad[mode_idx][pulse_idx][j] ≈
                        grad_pulses[j][pulse_idx].areaδ[mode_idx],
                          rtol=1e-2, atol=1e-2)
                end
            end
        end
