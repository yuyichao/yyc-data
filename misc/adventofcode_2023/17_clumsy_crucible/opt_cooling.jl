...
struct Site
    ...
    # by 1-3 steps.
    # We should maintain the invariance
    # min_cooling[1, i] <= min_cooling[2, i] <= min_cooling[3, i]
    min_cooling::Matrix{Int}
    function Site(cool)
        return new(cool, fill(typemax(Int), 3, 4))
    end
end
...
function load_lines(lines)
    ...
    sites[1, 1].min_cooling[:, RIGHT] .= 0
    sites[1, 1].min_cooling[:, DOWN] .= 0
    return sites
end
