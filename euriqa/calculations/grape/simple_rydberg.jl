...
struct PolyPMSeq{N,M}
end
function PolyPMSeq{N,M}(; opt_angle=false, kws...) where {N,M}
    ps = PMRampSeq{N}()
    model = SeqModel(ps; kws...)
    @variable(model.m, total_angle >= 0)
    @variable(model.m, orders[i=1:M + 1])
    fid_args = model.fid_args
    function phase_func(i)
        mid = N / 2
        x = (i - mid) / (N / 2)
        return evalpoly(x, orders)
    end
    start_phase = phase_func(0)
    angle = total_angle / N
    for i in 1:N
        end_phase = phase_func(i)
        push!(fid_args, angle)
        push!(fid_args, start_phase)
        push!(fid_args, end_phase - start_phase)
        start_phase = end_phase
