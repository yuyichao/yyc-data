...
struct Optimizer{NSeg,AreaObj,Sum}
    ωs::Vector{Float64}
    ηs::Vector{Float64}
    bij::Matrix{Float64}
    ωs2::Vector{Float64}
    ...
    function Optimizer{NSeg}(ωs, ηs, bij, ωs2;
                             tmin, tmax, ntimes=11,
        ...
        dis_weights = [fill(1, nions); fill(0.005, nions * 2); fill(0.001, nions)]
        disδ_weights = [fill(0.01, nions); fill(0, nions * 2); fill(0.000001, nions)]
        ...
        for i in 1:nions
            push!(area_targets, Opts.AreaTarget(nions * 3 + i, target=0,
                                                area_weights=[0.1]))
        end
        ...
        # Opts.set_bound!(area_tracker, area_obj.param.τ, 0.1, 3)
        ...
        return new{NSeg,typeof(area_obj),typeof(s)}(
            ωs, ηs, bij, ωs2, modes,
            tmin, tmax, ntimes, Ωmax,
            area_obj, area_tracker, area_opt,
            s, Candidate[], Vector{Float64}(undef, nargs),
            Vector{Float64}(undef, NSeg * 5)
        )
    end
end

function get_mode_weight!(o::Optimizer, ion1, ion2, weights)
    nions = length(o.ωs)
    for i in 1:nions
        weights[i] = o.bij[i, ion1] * o.bij[i, ion2] * o.ηs[i]^2
    end
    return weights
end

function areaopt_update_bounds!(o::Optimizer)
