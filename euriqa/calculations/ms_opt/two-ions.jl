...
args = Opts.gen_args(model, nseg,
                     ...
                     amp=Opts.AmpSpec(sym=false, mid_order=8, end_order=2))
...
for Ω in args.Ωs.poly
    push!(tracker, Ω, 0.05, 0.5)
end
for ω in args.ωs
    push!(tracker, ω, 1.0, 4.0)
end
obj = @NLexpression(model, (dis + disδ + 1e-15) / (1 + area^2))
...
@time for _ in 1:10000
    ...
    if value(obj) < best_obj
        best_obj = value(obj)
        @show best_obj, value(dis), value(disδ), value(cdis), value(area), value(areaδ), value(all_areaδ)
