...
% we need to clearly distinguish between the ions that needs to be measured,
% which we'll call the auxiliary ions, and the ions that aren't,
...
% This is also similar to how some neutral atom experiments
...
% An example is shown here, in which only one of the ion
% ends up being in the metastable state, despite requiring only global drive there.
% This can of course be customized as needed for each qudit systems.

% This second way of individual control of metastable state,
% which we called the qubit rotation implementation,
% does have it drawback since it usually drive all ions to the metastable state.
...
% since the main selling point of the hands-off methods is to not touch the data ions,
% having to drive all ions to the metastable state defeats the point
% and it doesn't usually make sense to have this combination.
