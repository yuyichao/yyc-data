...
\begin{frame}{$^{171}\text{Yb}^+$ qubit}
  \begin{center}
    \begin{columns}
      ...
      \begin{block}{}
        \begin{itemize}
        \item Long coherence time: $T_2\approx1 \text{hr}$\\
          {\scriptsize Wang, et al., Nat Commun 12, 233 (2021)}\\
          \vspace{1em}
        \item High fidelity state preparation: $> 99.9\%$ in $\approx 10 \mathrm{\mu s}$
        \item High speed and high fidelity readout: $> 99.3\%$ in $\approx 100 \mathrm{\mu s}$\\
          \vspace{0.5em}
