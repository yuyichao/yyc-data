...
\title[MCMR with \textit{omg}]{Mid-circuit measurement and reset using \textit{omg} architecture in trapped-ion quantum computer}
\date{Jan. 16, 2026}
\author[Yichao Yu]{Yichao Yu\\
  \vspace{0.5cm}
  {\footnotesize Keqin Yan, Vivian Zhang, Debopriyo Biswas, Bahaa Harraz,}\\
  {\footnotesize Crystal Noel, Christopher R Monroe and Alexander Kozhanov}}
\institute[Duke Quantum Center]{Duke Quantum Center, Duke University}
  ...
  \usebackgroundtemplate{
    \begin{tikzpicture}[remember picture,overlay]
      \begin{scope}[shift=(current page.center)]
        \node[outer sep=0,inner sep=0] at (0, 0) {\includegraphics[width=\paperwidth]{imgs/LabPicture_bg.jpg}};
...
\section{Brassboard/Gold system}
\begin{frame}{Quantum computers with individual Raman addressing at DQC}
  \begin{center}
    \begin{tikzpicture}
...
\begin{frame}{$2^{\text{nd}}$ gen EURIQA: Imaging system}
  \begin{center}
    \begin{tikzpicture}
      ...
      \visible<4->{
        \node at (2.8, -0.6) {\includegraphics[width=6cm]{imgs/pmt-array.png}};
      }
    ...
    \begin{tikzpicture}
      ...
      \node[text width=8cm] at (0, 3) {
        ...
        \item<3-> Individual addressing\\
          Nearest neighbor crosstalk: $1-2\%$.\\
          ($0.1 - 0.2\%$ with cancellation)
        \end{itemize}
      };
      \visible<2->{
...
\section{Mid-circuit measurement and reset (MCMR)}

% Like many others, experiments on our system typically follow
% the standard procedure of most AMO experiments up to this point.
% We start by initializing state with optical pumping, do some coherent evolution,
% and finally detect the state in the end with optical fluorescence.

% While this type of operation works well,
    ...
    \begin{tikzpicture}
      ...
      \visible<2-> {
        \begin{scope}[shift={(0cm, -3.5cm)}]
          ...
          (C1) at (0, 2.25) {Conditional\\operation};
...
% we need to clearly distinguish between the ions that need to be measured,
% which we'll call the auxiliary ions, and the ions that should be preserved,
% which will be called the data ions.
% Due to the broad linewidth of the transition typically used here,
% it can be really challenging to prevent scattering
% and loss of coherence on the data ion
% caused by any kind of crosstalk from the auxiliary.

% Still, for ion systems, this isolation has been realized,
...
% Both of these approaches, however, add significant complexity,
% and in the case of shuttling, time cost, to the experiment.
% So instead, we developed a family of techniques that can be added with low overhead
...
% This is also similar to how some neutral atom experiments implements it recently.
      ...
      \visible<2->{
        \node[align=center,text width=5cm,inner sep=0,outer sep=0] at (6, 1.3) {
          \begin{block}{}
            \begin{itemize}
            \item Shuttling\\
              \vspace{-0.1cm}\scalebox{0.6}{Nature Physics 19, 1 (2023)}
            \item Multi-species\\
              \vspace{-0.1cm}\scalebox{0.6}{Science 309, 749 (2005)}
            \item<3-> Metastable states\\
              \scalebox{0.9}{\textit{omg}-architecture}\\
              \vspace{-0.1cm}\scalebox{0.6}{APL 119, 214002 (2021)}\\
              \vspace{-0.1cm}\scalebox{0.6}{PRX 13, 041035 (2023)}
            \end{itemize}
          \end{block}
        };
      }
      \visible<3->{
        \node[align=left,text width=6cm,inner sep=0,outer sep=0] at (6, -2.4) {
          \begin{itemize}
          ...
          \item\makebox[0.7cm][r]{Ba$^+$}: D$_{3/2}$ \scalebox{0.85}{($80$ s)},
...
% Instead, all the distinctions between ion types are done with
...
% with the individual gate control beams.
% The dressing can selectively shift the resonance of
% the metastable transition allowing us to drive only the ions we want.
% Since the 355nm gate operation laser we use generates minimum scalar light shift
% to improve gate fidelity, we had to use a near resonance Raman dressing
% to produce an appriaciable shift.
...
% The way this works is to treat the qubit states
...
% individual *qubit* control on any two of the levels,
% combined with global operations connecting to other ones.
% This kind of sequence can of course be customized as needed for each qudit systems,
% but as an example,
% this sequence leaves only one of the ion to metastable state in the end,
% despite requiring only global drive there.

% This second way of individual control of metastable state,
% which we called the qubit rotation implementation,
% does have it drawback since it usually involves driving all ions to the metastable state.
% As you can see in this table showing the combination of the different methods,
% while dressing can be applied equally well to both of the MCMR methods,
% the combination of qubit-rotation with the hands-off methods usually doesn't make sense
% since driving everything to metastable state defeats the whole point of
% "hands-off the data".
...
% in the D3/2 manifold of Yb171 ions which has a coherence time of 10ms in our system
...
% and the error on top of that for dressing
...
\begin{frame}{}
  \begin{center}
    \begin{tikzpicture}
      \visible<2-> {
        \node[below,align=center,left] at (10.2, 5.5)
        {\usebeamercolor[fg]{frametitle}{\bf Mid-circuit measurement (shelving)}};
      }
      \visible<1-> {
        \begin{scope}[shift={(0, 1)}]
          ...
          \node[pyplotc0] at (1.9, 0.8) {\scalebox{0.7}{$t_2^{|D\rangle}\approx10\ \mathrm{ms}$}};
          \node at (1, 4) {\scalebox{1}{$^{171}\mathrm{Yb}^+$ levels}};
        \end{scope}
      }
      \visible<2-> {
        \node at (6.95, 5.0) {\scalebox{0.9}{Aux Detection}};
        \node at (6.7, 3.1) {\includegraphics[width=4.8cm]{imgs/detect_aux_rabi.pdf}};
      }
      \visible<3-> {
        \node at (6.95, 0.9) {\scalebox{0.9}{Data Coherence}};
        \node at (6.7, -1.0) {\includegraphics[width=4.8cm]{imgs/detect_data_ramsey_square.pdf}};
        \node[text width=5cm,align=left] at (1.2, -1.5) {
...
% We then demonstrated mid-circuit reset,
% this time using the hands-off method with dressing.
% Here the goal is simply to selectively drive one of the qubit states of the auxiliary
...
% coupling to the motional mode as well as Raman crosstalk,
% especially since the ion wasn't fully cooled for this experiment.

\begin{frame}{Mid-circuit reset (hands-off)}
  \begin{center}
    \vspace{-0.6cm}
    \begin{tikzpicture}
